...
Bugs should be filled on Launchpad (not GitHub) with the tag `elasticsearch`:
...
The *OpenStack Development Mailing List* is the prefered way to communicate:
   ...
   Send an email to `openstack-dev@lists.openstack.org` with the subject prefixed by `[fuel][plugins][elasticsearch]`
